...
//   ChevronRight, BatteryLow, Terminal, ShieldCheck,
// } from "lucide-react";

// import Sidebar from "../../components/common/Sidebar";
// import Topbar from "../../components/common/Topbar";
// import { Toast, useToast } from "../../components/common/Toast";
// import { S, StatusDot } from '../../components/common/ui';
...
//     { label: "Command Center", desc: "Send commands to devices", icon: Terminal, color: C.purple400, nav: "commands" },
...
import {
  Activity, LogOut, Shield,
  Smartphone, Settings, CheckCircle2, XCircle, RefreshCw, Plus,
  ChevronRight, BatteryLow, Terminal, ShieldCheck, BarChart3,
} from "lucide-react";
...
// ─── Device Detail Page (UPDATED - Removed Commands and Event Logs) ───────────
function DeviceDetailPage({ device, onBack }) {
  const battery = device?.battery_level ?? null;

  return (
    <div style={{ flex: 1, overflow: "auto", padding: "2rem" }}>
      <div style={{ display: "flex", alignItems: "center", gap: "1rem", marginBottom: "1.75rem" }}>
        <button onClick={onBack} style={{ ...S.btnSecondary, padding: "0.5rem 0.875rem" }}>← Back to Devices</button>
        <div>
          <h3 style={{ fontSize: "1.1rem", fontWeight: 700, color: C.text50, margin: "0 0 0.2rem" }}>{device?.device_name || "Device"}</h3>
          <p style={{ fontSize: "0.72rem", color: C.text500, margin: 0, fontFamily: "monospace" }}>ID: {device?.device_id}</p>
        </div>
      ...
      <div style={{ 
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", marginBottom: "1.5rem" }}>
          <Smartphone size={18} color={C.cyan400} />
          <p style={{ fontSize: "0.75rem", fontWeight: 600, color: C.text500, margin: 0, textTransform: "uppercase", letterSpacing: "0.08em" }}>
            Device Information
          </p>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: "1rem" }}>
          {[
            ...
            ["Enrolled Date", device?.created_at ? new Date(device.created_at).toLocaleDateString() : "—"],
          ].map(([label, value]) => (
            <div key={label} style={{ 
              padding: "0.75rem", 
              ...
              <div style={{ fontSize: "0.7rem", color: C.text500, marginBottom: "0.25rem" }}>{label}</div>
              <div style={{ fontSize: "0.85rem", color: C.text300, fontFamily: "monospace" }}>{value}</div>
            </div>
          ))}
        </div>
        ...
        {battery !== null && (
          <div style={{ marginTop: "1.5rem" }}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "0.5rem" }}>
              <span style={{ fontSize: "0.78rem", color: C.text500 }}>Battery Level</span>
              <span style={{ fontFamily: "monospace", fontSize: "0.85rem", fontWeight: 600, color: battery < 20 ? C.red400 : battery < 50 ? C.yellow400 : C.emerald400 }}>
                {battery}%
              </span>
            </div>
            <div style={{ height: 8, backgroundColor: C.bg700, borderRadius: 9999, overflow: "hidden" }}>
              <div style={{ 
                height: "100%", 
                width: `${battery}%`, 
                backgroundColor: battery < 20 ? C.red500 : battery < 50 ? C.yellow500 : C.emerald500, 
                borderRadius: 9999,
      ...
      <div style={{ 
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", marginBottom: "1.5rem" }}>
          <BarChart3 size={18} color={C.cyan400} />
          <p style={{ fontSize: "0.75rem", fontWeight: 600, color: C.text500, margin: 0, textTransform: "uppercase", letterSpacing: "0.08em" }}>
            Device Statistics
          </p>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "1rem" }}>
          <div style={{ textAlign: "center", padding: "1rem", backgroundColor: "rgba(15, 23, 42, 0.5)", borderRadius: "8px" }}>
            <div style={{ fontSize: "1.5rem", fontWeight: 700, color: C.cyan400 }}>—</div>
            <div style={{ fontSize: "0.7rem", color: C.text500, marginTop: "0.25rem" }}>Commands Sent</div>
          </div>
          <div style={{ textAlign: "center", padding: "1rem", backgroundColor: "rgba(15, 23, 42, 0.5)", borderRadius: "8px" }}>
            <div style={{ fontSize: "1.5rem", fontWeight: 700, color: C.emerald400 }}>—</div>
            <div style={{ fontSize: "0.7rem", color: C.text500, marginTop: "0.25rem" }}>Success Rate</div>
          </div>
          <div style={{ textAlign: "center", padding: "1rem", backgroundColor: "rgba(15, 23, 42, 0.5)", borderRadius: "8px" }}>
            <div style={{ fontSize: "1.5rem", fontWeight: 700, color: C.yellow400 }}>—</div>
            <div style={{ fontSize: "0.7rem", color: C.text500, marginTop: "0.25rem" }}>Avg Response</div>
          </div>
        </div>
        ...
        <div style={{ 
          ...
          borderRadius: "8px",
          textAlign: "center",
          border: `1px dashed ${C.cyan400}30`
        }}>
          <p style={{ fontSize: "0.7rem", color: C.text500, margin: 0 }}>
            💡 To send commands to this device, go to the <strong style={{ color: C.cyan400 }}>Command Center</strong> and select this device
          </p>
        </div>
      </div>
    </div>
      ...
      <div style={{ position: "relative", display: "flex", height: "100vh", overflow: "hidden" }}>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", marginLeft: sidebarOpen ? 220 : 68, transition: "margin-left 0.3s ease", overflow: "hidden" }}>
          ...
          <div style={{ flex: 1, overflow: "auto", display: "flex", flexDirection: "column" }}>
            {activePage === "dashboard" && <DashboardHome devices={devices} onNavigate={goTo} />}
            {activePage === "devices" && <DevicesPage onDeviceClick={d => { setSelectedDevice(d); setActivePage("device-detail"); }} toast={toast} />}
            {activePage === "device-detail" && <DeviceDetailPage device={selectedDevice} onBack={() => goTo("devices")} />}
            {activePage === "commands" && <CommandsPage />}
          </div>
        </div>
      </div>
